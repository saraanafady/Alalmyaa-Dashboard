...
const CategoriesPage = () => {
                ...
                .map(async (subcategory) => {
                  ...
                  try {
                    const { data: response } = await axios.get(
                      `${base_url}/api/subsubcategory?subcategoryId=${subcategory._id}`,
                      { headers }
                    );
                    // Access the correct data structure
                    const subSubData = response.data || []; // Get the data array from response

                    const subSubcategories = Array.isArray(subSubData)
                      ? subSubData.map((subSub) => ({
                          ...subSub,
                          _id: subSub._id || subSub.id,
                          isActive: subSub.isActive ?? true,
      ...
      for (const category of categories) {
        for (const subcategory of category.subcategories || []) {
          ...
          try {
            ...
            console.error(
              `Error fetching sub-subcategories for ${subcategory._id}:`,
              error
            );
  ...
  const deleteSubSubcategoryMutation = useMutation({
    mutationFn: (id) => {
      ...
      return axios.delete(`${base_url}/api/subsubcategory/${id}`, { headers });
    },
    onSuccess: () => {
  ...
  const handleCategorySubmit = async (e) => {
    ...
    try {
      if (selectedCategory?._id) {
        // Check if _id exists for selectedCategory
  ...
  const renderSubSubcategories = (subcategory, category) => {
    ...
    const subSubList = (subSubcategories[subcategory._id] || []).filter(
      (subSub) =>
        subSub.subcategory === subcategory._id &&
        subSub.category === category._id
    );
