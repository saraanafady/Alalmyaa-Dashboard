        ...
        <div className="w-2/3 space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {product.category?.name?.en ? (
              <span className="font-medium">{product.category.name.en}</span>
            ) : null}
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className="w-14 h-14">
                  <img
                    ...
                    alt={option.colorName?.en || "Variant image"}
                    className="w-full h-full object-contain rounded cursor-pointer hover:opacity-90 transition-opacity"
                    onClick={() =>
                ...
                <div>
                  <h4 className="font-medium">
                    Color: {option.colorName?.en || "No color specified"}
                  </h4>
                  <div className="flex gap-2 mt-1">
                    {option.variantImages?.map?.((img, imgIndex) => (
                      <img
                        key={imgIndex}
                        src={img.url || ""}
                        alt={`${option.colorName?.en || "Variant"} ${
                          imgIndex + 1
                        }`}
                        className="w-8 h-8 object-contain rounded cursor-pointer hover:opacity-75 transition-opacity"
                        onClick={() => handleImageClick(img.url)}
          ...
          <div className="grid grid-cols-2 gap-4">
            {product.specifications.map((spec, index) => (
              <div key={index} className="bg-gray-50 p-3 rounded">
                <span className="text-gray-600">
                  {spec.name?.en || "Specification"}:
                </span>
                <span className="ml-2 font-medium">
                  {spec.value || "N/A"} {spec.unit?.en || ""}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-2 bg-white p-3 rounded-md border-2 border-gray-300">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: product.details?.en || "No description available",
          }}
        />
      </div>
