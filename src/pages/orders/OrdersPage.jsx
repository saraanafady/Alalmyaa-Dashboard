...
const OrdersPage = () => {
        ...
        response.data.data.orders.map(async (order) => {
          try {
            ...
            if (order.user && typeof order.user === 'string') {
              const userResponse = await axios.get(`${base_url}/api/users/${order.user}`, {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
              });
  ...
  const updateOrderStatusMutation = useMutation({
    mutationFn: async ({ orderId, status }) => {
      try {
        ...
        if (error.response?.status === 401) {
          ...
          throw new Error(error.response?.data?.message || "Server error while updating order status. Please try again.");
        } else {
    ...
    onError: (error) => {
      ...
      if (error.message.includes("session has expired") || error.message.includes("not authenticated")) {
        navigate("/login");
      }
    },
  });
                ...
                <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <div className="flex items-center">
                      <div className="h-10 w-10 flex-shrink-0">
                        <img
                          ...
                          src={order.user?.profilePicture || `https://ui-avatars.com/api/?name=${encodeURIComponent(`${order.user?.firstName || ''} ${order.user?.lastName || ''}`)}`}
                          alt={`${order.user?.firstName} ${order.user?.lastName}`}
                          onError={(e) => {
                            e.target.onerror = null;
                            e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(`${order.user?.firstName || ''} ${order.user?.lastName || ''}`)}`;
                          }}
                        />
                      </div>
                      <div className="ml-4">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <select
                      ...
                      disabled={updatingOrderId === order._id || !user || user.role !== "admin"}
                      className={`text-sm rounded-full px-2 py-1 font-semibold ${
                        statusColors[order.status] || statusColors.pending
                      } ${(updatingOrderId === order._id || !user || user.role !== "admin") ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      <option value="pending">Pending</option>
                      <option value="processing">Processing</option>
                      <option value="shipped">Shipped</option>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`text-sm rounded-full px-2 py-1 font-semibold ${
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Customer: {selectedOrder.user?.firstName || 'N/A'} {selectedOrder.user?.lastName || 'N/A'}
                </p>
              </div>
              <button
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex items-start space-x-4">
                  <img
                    src={selectedOrder.user?.profilePicture || `https://ui-avatars.com/api/?name=${encodeURIComponent(`${selectedOrder.user?.firstName || ''} ${selectedOrder.user?.lastName || ''}`)}`}
                    alt={`${selectedOrder.user?.firstName} ${selectedOrder.user?.lastName}`}
                    className="h-20 w-20 rounded-full object-cover"
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(`${selectedOrder.user?.firstName || ''} ${selectedOrder.user?.lastName || ''}`)}`;
                    }}
                  />
                  <div className="flex-1">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm font-medium text-gray-900">
                          Name: {selectedOrder.user?.firstName || 'N/A'} {selectedOrder.user?.lastName || 'N/A'}
                        </p>
                      ...
                      <div>
                        <p className="text-sm text-gray-500">
                          Member Since: {selectedOrder.user?.createdAt ? new Date(selectedOrder.user.createdAt).toLocaleDateString() : 'N/A'}
                        </p>
                        <p className="text-sm text-gray-500">
                          Total Orders: {selectedOrder.user?.ordersCount || 0}
                        </p>
                        <p className="text-sm text-gray-500">
                          Total Spent: ${formatCurrency(selectedOrder.user?.totalSpent)}
                        </p>
                      </div>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {selectedOrder.deliveredAt && (
                    <p className="text-sm text-gray-500">
                      Delivered At: {selectedOrder.deliveredAt ? new Date(selectedOrder.deliveredAt).toLocaleDateString() : 'N/A'}
                    </p>
                  )}
                </div>
              </div>
