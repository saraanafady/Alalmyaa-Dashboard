      ...
      try {
        ...
        if (error.response) {
          if (error.response.status === 404) {
            throw new Error("Order not found. Please refresh the page and try again.");
          ...
          <div className="flex items-center">
            <div className="h-10 w-10 flex-shrink-0">
              <img
                ...
                src={customer.profilePicture || `https://ui-avatars.com/api/?name=${encodeURIComponent(`${customer.firstName || ''} ${customer.lastName || ''}`)}`}
    ...
    },
    {
      ...
      render: (order) => (
        <select
          ...
          disabled={updatingOrderId === order._id || !adminUser || adminUser.role !== "admin"}
          className={`text-sm rounded-full px-2 py-1 font-semibold ${
            statusColors[order.status] || statusColors.pending
          } ${(updatingOrderId === order._id || !adminUser || adminUser.role !== "admin") ? 'opacity-50 cursor-not-allowed' : ''}`}
        >
                      ...
                      <div className="flex-1">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <p className="text-sm font-medium text-gray-900">
                              Name: {customer.firstName || 'N/A'} {customer.lastName || 'N/A'}
                            </p>
                        ...
                        {selectedOrder.shippingAddress?.address && (
                          <p className="text-sm text-gray-500 mt-2">
                            Address: {selectedOrder.shippingAddress.address}, {selectedOrder.shippingAddress.city}, {selectedOrder.shippingAddress.postalCode}, {selectedOrder.shippingAddress.country}
                          </p>
                        )}
                      </div>
                    </>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="mt-1 space-y-1">
                  ...
                  <p className="text-sm text-gray-900">
                    Status: <Badge variant={selectedOrder.paymentStatus === 'paid' ? 'success' : 'warning'}>
              ...
              <div>
                ...
                <div className="mt-1 space-y-1">
                  ...
                  {selectedOrder.deliveredAt && (
                    <p className="text-sm text-gray-900">
                      Delivered At: {new Date(selectedOrder.deliveredAt).toLocaleDateString()}
                    </p>
                  )}
                </div>
              </div>
